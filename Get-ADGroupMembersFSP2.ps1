...
function Get-AllMembersFromGroup {
    ...
    process {
        try {
            if ($Credential) {
                $MemberList = (Get-ADGroup -Identity $GroupName -Server $DomainFQDN -Credential $Credential -Properties Members).Members
            }
            else {
            ...
            foreach ($memberDN in $MemberList) {
                ...
                if ($Recursive -and $AdObject.ObjectClass -eq "group" -and -not ($ProcessedGroups -contains $AdObject.DistinguishedName)) {
                    $ProcessedGroups += $AdObject.DistinguishedName
                    Write-Debug "Recursively enumerating group: $($AdObject.DistinguishedName)"
                    $nestedMembers = Get-AllMembersFromGroup -GroupName $AdObject.DistinguishedName -DomainFQDN $DomainFromUserDN -Credential $Credential -Recursive
                    $ObjectInfo += $nestedMembers
                }
            }
        }
        catch {
            ...
            return ($ObjectInfo | Sort-Object -Property DistinguishedName -Unique)
        }
    }
